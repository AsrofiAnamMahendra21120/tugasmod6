...
export function ControlScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setError(null);
    try {
      // require auth token to call protected endpoint
      if (!authToken) {
        setError("You must be logged in to save thresholds.");
        return;
      }
      await Api.withToken(authToken).createThreshold({ value: valueNumber, note });
      setNote("");
      await fetchHistory();
    } catch (err) {
      setError(err.message);
